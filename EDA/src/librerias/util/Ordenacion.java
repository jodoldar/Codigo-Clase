 // Comprueba que está en el lugar correcto: 
    ...
    */
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v, int i, int f){
        ...
        if(i==f){
            ...
            int m = (i + f) / 2;
            res = merge2(mergeSort2(v,i,m),mergeSort2(v,m+1,f));
        }*/
        if (i < f ) {
            if (i+1>=f){
                T[] res = (T[]) new Comparable[2];
                if(v[i].compareTo(v[i+1])<0){
                    res[0] = v[i]; res[1] = v[i+1];
                }else{
                    res[0] = v[i+1]; res[1] = v[i];
                }
                return res;
            }else{
                int m = (i + f) / 2;
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2){
        ...
        while (a <= v1.length-1) { res[k++] = v1[a++]; }
        while (b <= v2.length-1) { res[k++] = v2[b++]; }
