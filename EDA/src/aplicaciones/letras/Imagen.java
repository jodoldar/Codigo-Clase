...
public class Imagen {
    ...
    */ 
    ...
    private int valorHashSumaPonderadaTodos() {
        final int Pixel_BASE = 255;
        int res = 0;
        for(int i=0;i<alto;i++){
            for(int j=0;j<ancho;j++){
                res = (res*Pixel_BASE) + pixels[i][j].hashCode();
            }
        }
        return res;
    }
    ...
    private int valorHashSuma3() {
        int res = 0;
        int mitadAlto = alto/2; int mitadAncho = ancho/2;
        for(int i=mitadAlto-1;i<=mitadAlto+1;i++){
            for(int j=mitadAncho-1;j<=mitadAncho+1;j++){
                res += pixels[i][j].hashCode();
            }
        }
        return res;
