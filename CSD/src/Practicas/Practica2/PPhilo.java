...
public class PPhilo extends JPanel implements ActionListener {
    private Vector<State> state = new Vector<State>();
    private JList text;
    private Box box;
    ...
    void simulate(String s, int delay) {
        ...
        Table t; 
        switch (type) {
        case 0: System.out.println("Deadlock not prevented");
                t = new RegularTable(log);
                for (i=0; i<5; i++) philo[i]=new Philo(i,cycles,delay,t); 
                break;
        case 1: System.out.println("Assimetry (last/but last)");
                t = new RegularTable(log);
                for (i=0; i<5; i++) philo[i]=new LefthandPhilo(i,cycles,delay,t); 
                break;
        case 2: System.out.println("Assimetry (even/odd)");
                t = new RegularTable(log);
                for (i=0; i<5; i++) philo[i]=new LefthandPhilo(i,cycles,delay,t); 
                break;
        case 3: System.out.println("Both or None");
                t = new BothNoneTable(log);
                for (i=0; i<5; i++) philo[i]=new PhiloBothOrNone(i,cycles,delay,t); 
                break;
        case 4: System.out.println("Limited capacity Dining Room");
