...
void encaja(Imagen *ima)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = i + 1; j < ima->alto; j++) {
            distancia = 0;
            #pragma omp parallel reduction(+:distancia) private (id,x)
            {
                id = omp_get_thread_num();
                salt = omp_get_num_threads();
                for (x = id; x < ima->ancho && distancia<=distancia_minima; x+=salt){
...
int main(int argc, char *argv[])
{
    ...
    *salida = "Lenna-e4-pX.ppm";
    ...
    encaja(&ima);
    t = omp_get_wtime() - t;
    printf("Tiempo empleado en la imagen: %f\n",t);
